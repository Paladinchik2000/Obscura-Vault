...
переведены в hex; Chrome сверен с реальным APK на эмуляторе. Samsung Internet
в списке нет: проверить его хэш было неоткуда, поэтому он считается обычным
приложением. Появится телефон — снять `apksigner verify --print-certs` и
добавить.
